...
void drawDots()
{
	...
	{
		/*
		 * Reset the colour pointers for every dot.
		 *
		 * They are declared outside this loop, so once a dot carried an explicit
		 * colour they kept pointing at it and every later dot inherited it. On a
		 * curve segment (1,2,3) that meant landmark 1 drew red (it precedes the
		 * slider), landmark 2 set the pointer to blue, and landmark 3 rendered blue
		 * even though R had sent it the (-1,-1,-1) restore sentinel.
		 */
		c = &defaultDotColor;
...
void drawCurves()
{
	...
	/*
	 * Bind the curve slice to the specimen actually on screen, the way drawDots()
	 * and drawAnchors() already do with dotSetArrayIndex(model_index).
	 *
	 * drawCurves() used to trust whatever get_curve_slice_id() happened to hold.
	 * The "add curve" branch walks curveSetArrayIndex(cc) over every slice, so it
	 * exits with the index on the LAST specimen. A redraw issued right after
	 * therefore asked for the wrong specimen's curve and drew nothing, which is
	 * why a new segment only appeared once a tab switch ran showPicture() and
	 * reset the index.
	 */
