...
- [ ] **BLD-01**: A tri-platform CMake build produces `tkogl2.dylib` on macOS (Mach-O `-dynamiclib`), linking `-framework OpenGL/AppKit/Foundation`, without breaking the Windows MSVC build — *Windows half VERIFIED 2026-07-16 (restructured CMake rebuilt `tkogl2.dll` clean, no unresolved Tk symbols, renders unchanged incl. a 6-specimen `.dgt`); macOS `.dylib` build still pending — first Mac build lands in Phase 4*
...
| BLD-01 | Phase 3 | Windows half verified 2026-07-16 (builds + renders unchanged); macOS `.dylib` build in Phase 4 |
