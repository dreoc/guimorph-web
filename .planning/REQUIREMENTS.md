...
- [ ] **WEB-00**: three.js, its OrbitControls and PLYLoader addons, and `three-mesh-bvh` are vendored into `inst/htmlwidgets/` as a **single classic-script bundle** with licence files and a version manifest, produced by the pinned, re-runnable toolchain in `scripts/vendor/`. A minimal R-side wrapper (scene, camera, orbit, resize, reset view) writes a plain HTML page and opens it with `browseURL()`, with no network access and **without adding `htmlwidgets` back to Imports**. Owned by Phase 1 because PLT-01 is the first thing that needs it; WEB-02 and PICK-01 reuse the same bundle and wrapper
  - *Classic script, not ES modules:* three.js ships ES modules only from 0.160, and ES modules are CORS-blocked on `file://`. Bundling to an IIFE is what lets Phase 1 stay transport-free while the same bundle works unchanged over `httpuv` from Phase 2
